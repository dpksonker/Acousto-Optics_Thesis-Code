function [Bound,Tissue,AO,M0,M1] = AO_09MHz_water(Bound,Tissue,n,n_photons,W,AO,M0,M1)
%% The function AO_09MHz_water is the main Monte Carlo simulation function it calculates the total phase modulations due to both AO mechanisms and stores the phase modulations 
...
for np=1:n_photons                                                              % Main loop to simulate the number of photons
    ...
    Pn_cosi = single(0);
    Pn_sini = single(0);
    Pd_cosj = single(0);                                                    % Variables for storing phase accumulations at each photon step and scattering
    Pd_sinj = single(0);
