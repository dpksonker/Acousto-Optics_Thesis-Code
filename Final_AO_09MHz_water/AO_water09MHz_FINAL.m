...
AO.Pressure = n_p/4.9383e3;                                                 % normalize the US pressure to 1 kPa
AO.Phase = n_ph;                                                             % upload the phase data
AO.ux = u_xx;
AO.uy = u_yy;                                                               % particle velocity in y direction
AO.uz = u_zz;
...
Tissue.f0 = single(0.9e6);                                                 % Acoustic Frequency
...
%  [Bound,Tissue,AO,M0,M1] = AO_09MHz_water(Bound,Tissue,n,n_photons,W,AO,M0,M1);       % Main calling function to simulate the Monte Carlo
[Bound,Tissue,AO,M0,M1] = AO_09MHz_water_mex(Bound,Tissue,n,n_photons,W,AO,M0,M1);

Data_09MHzFUS_water(1:9,:) = {Tissue.mod1(1:Tissue.count_D1,:),"Tissue.Mod1";Tissue.mod2(1:Tissue.count_D2,:),"Tissue.Mod2";Tissue.mod3(1:Tissue.count_D3,:),"Tissue.Mod3";Tissue.phi1(1:Tissue.count_D1,:),"Tissue.phi1";Tissue.phi2(1:Tissue.count_D2,:),"Tissue.phi2";Tissue.phi3(1:Tissue.count_D3,:),"Tissue.phi3";...
...
colorbar
saveas(gcf,'FUS_Unmodulated09MHz.fig')
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
figure (3)
...
ylabel('z (mm)','FontSize',45);
colorbar
saveas(gcf,'FUS_modulated09MHz.fig')
...
ylabel('z (mm)','FontSize',45);
colorbar
saveas(gcf,'FUS_MDepth09MHz.fig')
...
% phi_ph = Tissue.phi1(1:Tissue.count_D1,2);
% 
% argg = ((2*pi*Tissue.f0*time_arr_p)' - phi_ph);
% ph_tot = real(phi_mag.*exp(1i*(argg)));
% 
% f_light = 3e8/lambda;
% 
% Et = wt.*exp(1i*(2*pi*f_light*time_arr_p + ph_tot));
% 
% figure(33)
% plot(abs(Et))
% 
