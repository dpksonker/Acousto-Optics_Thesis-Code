function [Photon,Tissue] = CrossDOWN(Tissue,Photon,n)
...
% Tissue: saved weight of photon which went out of the medium as Transmittance
%%
 ...
 if ee <= Ref || Ref == 1                                                  % Check for reflection at boundary
     ...
     if Photon.Layer == length(n)-2                                                 % Check if the photon is at top-most layer          
         ...
         [Photon,Tissue] = Transmittance(Photon,Tissue);                   % Then call the function to check for Transmittance from last layer
