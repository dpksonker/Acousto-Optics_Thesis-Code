...
r_p1  = sqrt((Photon.x - D1)*(Photon.x - D1) + (Photon.y - D1)*(Photon.y - D1));      % Calculate the distance of a photon from the detector D1

if  r_p1 <= Tissue.Radius_det                                                      % Check if the photon is inside the detector D1
    Tissue.count_D1 = Tissue.count_D1 + 1; 
    
    Tissue.mod1(Tissue.count_D1,1) = Photon.id;                                 % Save the photon id
    Tissue.mod1(Tissue.count_D1,2:end) = mod_dem;                               % Save the modulation magnitude
    Tissue.phi1(Tissue.count_D1,1) = Photon.id;                                  
    Tissue.phi1(Tissue.count_D1,2:end) = atan(phi_dem);                         % Save the phase of the phase modulation        
    
    Tissue.Det1(Tissue.count_D1,1) =  Photon.id;                                % Save the photon id
    Tissue.Det1(Tissue.count_D1,2) =  Photon.weight;                               % Save the weight of the detected photon
    Tissue.Det1(Tissue.count_D1,3:end) = ppl_dem;                                  % Save the partial path length 
...
if  r_p2 <= Tissue.Radius_det 
    ...
    Tissue.mod2(Tissue.count_D2,2:end) = mod_dem;
    Tissue.phi2(Tissue.count_D2,1) = Photon.id;
    Tissue.phi2(Tissue.count_D2,2:end) = atan(phi_dem);

    Tissue.Det2(Tissue.count_D2,1) =  Photon.id;
    Tissue.Det2(Tissue.count_D2,2) =  Photon.weight;
    Tissue.Det2(Tissue.count_D2,3:end) = ppl_dem;
end
% 
...
if  r_p3 <= Tissue.Radius_det 
    ...
    Tissue.mod3(Tissue.count_D3,2:end) = mod_dem;
    Tissue.phi3(Tissue.count_D3,1) = Photon.id;
    Tissue.phi3(Tissue.count_D3,2:end) = atan(phi_dem);

    Tissue.Det3(Tissue.count_D3,1) =  Photon.id;
    Tissue.Det3(Tissue.count_D3,2) =  Photon.weight;
    Tissue.Det3(Tissue.count_D3,3:end) = ppl_dem;
end
